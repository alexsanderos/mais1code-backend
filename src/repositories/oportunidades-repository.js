...
exports.criarOportunidade = async(id, nome, depoimento) => {
    try {
...
exports.atualizarOportunidade = async(depoimento, id) => {
    try {
        return clientBase.query(`UPDATE oportunidades SET oportunidades = $1 WHERE id = $2`, [depoimento, id]);
    } catch (error) {
        console.error(error);
